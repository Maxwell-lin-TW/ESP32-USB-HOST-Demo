 ...
 * APP_EVENT_UNINSTALL  - Use for uninstall all usb host client.
 * APP_EVENT_HID_HOST   - HID Host Driver event, such as device connection/disconnection or input report.
 */
typedef enum hid_event_enum_s{
    APP_EVENT_UNINSTALL = 0,
    APP_EVENT_HID_HOST
} hid_event_enum_t;
 ...
 */
typedef struct hid_event_queue_s{
    hid_event_enum_t event_group;
...
void hid_event_callback(hid_host_device_handle_t hid_device_handle,
...
static void key_event_callback(key_event_t *key_event)
{
    ...
    if (KEY_STATE_PRESSED == key_event->state) {
        if (hid_keyboard_get_char(key_event->modifier,
            ...
            // ESP_LOGI(TAG, "Get Char 0x%02x", key_char);
            if(key_char < 0x7F){
 ...
 * @param[in] length  Length of input report data buffer
 */
static void hid_host_mouse_report_callback(const uint8_t *const data, const int length, uint8_t addr)
{
    static int x_pos = 0;
    static int y_pos = 0;
    ...
    if (length == sizeof(hid_mouse_input_report_boot_t)) {
        ...
        printf("addr=0x%02x: X: %06d\tY: %06d\t|%c|%c|\r\n",
            addr,
            x_pos, y_pos,
        ...
        printf("addr=0x%02x, reportID=0x%02X\n", addr, *data_ptr);
        ++data_ptr;
    ...
    case HID_HOST_INTERFACE_EVENT_INPUT_REPORT:
        ...
        if (HID_SUBCLASS_BOOT_INTERFACE == dev_params.sub_class) {
            if (HID_PROTOCOL_KEYBOARD == dev_params.proto) {
                ...
                hid_host_mouse_report_callback(data, data_length, dev_params.addr);
            }
        } else {
            ESP_LOGI(TAG, "addr=%02X", dev_params.addr);
...
esp_err_t hid_host_get_and_print_report_descriptor(hid_host_device_handle_t hid_device_handle){

    size_t get_report_desc_size = 0;
    uint8_t *get_report_desc_ptr = hid_host_get_report_descriptor(hid_device_handle, &get_report_desc_size);

    if( (get_report_desc_ptr != NULL) && (get_report_desc_size != 0) ){
        ESP_LOGI(TAG, "Print Report Descriptor");
        for( int i = 0; i < get_report_desc_size; i++){
            if( ( ( i % 16 ) == 0 ) && ( i != 0 )){
                printf("\n");
            }
            printf("0x%02X,", *get_report_desc_ptr);
            ++get_report_desc_ptr;
        }
        printf("\n");
        fflush(stdout);
                           ...
                           void *arg)
{
    ...
    case HID_HOST_DRIVER_EVENT_CONNECTED:
        ...
        ESP_ERROR_CHECK(hid_host_get_and_print_report_descriptor(hid_device_handle));
        if (HID_SUBCLASS_BOOT_INTERFACE == dev_params.sub_class) {
            ...
            ESP_LOGI(TAG, "dev_params.sub_class = %d", dev_params.sub_class);
        }
...
void usbhost_hid_Task(void *arg){
    ...
    hid_host_driver_config_t hid_host_config = {
        ...
        .stack_size = 4096,
        .core_id = 0,
        .callback = hid_event_callback,
        .callback_arg = NULL
    };
    ESP_ERROR_CHECK(hid_host_install(&hid_host_config));

    app_event_queue = xQueueCreate(10, sizeof(hid_event_queue_t));
    ...
    while(1){
        ...
        if (xQueueReceive(app_event_queue, &evt_queue, portMAX_DELAY)) {
            if (APP_EVENT_UNINSTALL == evt_queue.event_group) {
                ...
                if (lib_info.num_devices == 0) {
                    ...
                    uint8_t *get_addr_list = (uint8_t *)calloc(10, sizeof(uint8_t));
                    int get_total_len = 0;
                    if(usb_host_device_addr_list_fill(10, get_addr_list, &get_total_len)){
                        ESP_LOGE(TAG, "Get device list failed:%d", get_total_len);
                    }else{
                        while(get_total_len){
                            ESP_LOGI(TAG, "%02X", *get_addr_list);
                            --get_total_len;
                            ++get_addr_list;
                        }
                    }
...
//******************************************************************************
esp_err_t usbhost_hid_uninstall(void){

    hid_event_queue_t send_uninstall_event;

    send_uninstall_event.event_group = APP_EVENT_UNINSTALL;

    xQueueSend(app_event_queue, &send_uninstall_event, pdMS_TO_TICKS(100));

    vTaskDelay(pdMS_TO_TICKS(1000));

    if(app_event_queue != NULL){
        ESP_LOGI(TAG, "usbhost driver deinit failed");
        return ESP_FAIL;
    }else{
        ESP_LOGI(TAG, "usbhost driver deinit sucessfully");
        // usbhost_class_identify_Task_Delete();
        return ESP_OK;
    }
